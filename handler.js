...
export async function handler(chatUpdate) {
  ...
  try {
      ...
      return list
        .flatMap((item) => {
          ...
          if (Array.isArray(item))
            return normalizeJid(item[0]) ? [normalizeJid(item[0])] : [];
    ...
    // flags
    const isROwner =
      roownerList.includes(senderJid) || rownerList.includes(senderJid);
    const isOwner =
      isROwner || ownerList.includes(senderJid) || m.fromMe === true;
    const isPrems =
      isROwner || premsList.includes(senderJid) || !!(user && user.premium);
    ...
    const ___dirname =
      global.__dirname(import.meta.url, false) ||
    ...
    const botGroup =
      (m.isGroup
        ? participants.find((u) => conn.decodeJid(u.jid) == this.user.jid)
        : {}) || {};
    ...
    for (const name in global.plugins) {
      ...
      const __filename =
        global.__filename(import.meta.url, false) || join(___dirname, name);
      ...
      if (prefixMatch && (usedPrefix = prefixMatch.prefix)) {
        ...
        if (
          ...
          const participants = m.isGroup
            ? (
                await this.groupMetadata(m.chat).catch(() => ({
                  participants: [],
                }))
              ).participants
            : [];
...
global.dfail = (type, m, conn) => {
  ...
  const msg = {
    ...
    premium:
      "> `ⓘ ᥱs𝗍ᥱ ᥴ᥆mᥲᥒძ᥆ s᥆ᥣ᥆ sᥱ ⍴ᥙᥱძᥱ ᥙ𝗍іᥣіzᥲr ⍴᥆r ᥙsᥙᥲrі᥆s ⍴rᥱmіᥙm, ᥡ ⍴ᥲrᥲ mі ᥴrᥱᥲძ᥆r.`",
    ...
    botAdmin:
      "> `ⓘ ⍴ᥲrᥲ ⍴᥆ძᥱr ᥙsᥲr ᥱs𝗍ᥱ ᥴ᥆mᥲᥒძ᥆ ᥱs ᥒᥱᥴᥱsᥲrі᥆ 𝗊ᥙᥱ ᥡ᥆ sᥱᥲ ᥲძmіᥒ.`",
    unreg:
      "> `ⓘ ᥒᥱᥴᥱsі𝗍ᥲs ᥱs𝗍ᥲr rᥱgіs𝗍rᥲძ᥆(ᥲ) ⍴ᥲrᥲ ᥙsᥲr ᥱs𝗍ᥱ ᥴ᥆mᥲᥒძ᥆, ᥱsᥴrіᑲᥱ #rᥱg ⍴ᥲrᥲ rᥱgіs𝗍rᥲr𝗍ᥱ.`",
